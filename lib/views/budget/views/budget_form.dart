...
class _BudgetFormState extends State<BudgetForm> {
  ...
  get _totalKategori => _kategoris.fold<double>(
    0,
  ...
  bool _isKategoriValid() {
    ...
    if (_totalKategori < budgetAmount) {
      _showWarning(
        ...
        'Total nilai kategori tidak boleh kurang dari jumlah budget',
      );
      return false;
    }
    if (budgetAmount < _totalKategori) {
      _showWarning(
        'Budget Tidak Valid',
  ...
  // Fungsi untuk menampilkan form tambah kategori di modal bottom sheet
  void _showTambahKategoriForm() {
  ...
  void _showEditKategoriForm(KategoriModel kategori, int index) {
    final budgetAmount = double.tryParse(_amountController.text.trim()) ?? 0;
    showModalBottomSheet(
      isScrollControlled: true,
      context: context,
      builder: (context) {
        return BlocProvider.value(
          ...
          child: Padding(
            ...
            child: KategoriForm(
              kategori: kategori,
              onSubmit: (newKategori) {
                if (_totalKategori -
                        _kategoris[index].planned +
                        newKategori.planned >
                    budgetAmount) {
                  _showWarning(
                    'Jenis Transaksi Tidak Valid',
  ...
  // Fungsi untuk menghapus kategori dari list dan state
  void _deleteKategori(KategoriModel kategori, int index) {
    context.read<KategoriBloc>().add(DeleteKategoriEvent(kategori: kategori));
    setState(() {
      _kategoris.removeAt(index);
    });
  }
  ...
  // Widget untuk menampilkan daftar kategori dan tombol tambah kategori
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Text(
              'Jenis Transaksi Budget',
              style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
            ),
            const Spacer(),
            SaveButton(onPressed: _showTambahKategoriForm, label: "Tambah"),
          ],
        ),
        const SizedBox(height: 12),
        _kategoris.isEmpty
            ? const Text('Tidak ada Jenis Transaksi')
            : ListView.builder(
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemCount: _kategoris.length,
                itemBuilder: (context, index) {
                  final KategoriModel kategori = _kategoris[index];
                  return ListTile(
                    onTap: () => _showEditKategoriForm(kategori, index),
                    leading: CircleAvatar(
                      backgroundColor: kategori.color,
                      child: Text(
                        kategori.kategori.substring(0, 1).toUpperCase(),
                        style: const TextStyle(color: Colors.white),
                      ),
                    ),
                    title: Text(kategori.kategori),
                    subtitle: Text(Utils.toIDR(kategori.planned)),
                    shape: Border(
                      bottom: BorderSide(color: Colors.grey.shade300, width: 1),
                    ),
                    trailing: IconButton(
                      icon: const Icon(Icons.delete, color: Colors.red),
                      onPressed: () => _deleteKategori(kategori, index),
                    ),
                  );
                },
              ),
